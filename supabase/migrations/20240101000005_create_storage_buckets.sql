...
CREATE POLICY IF NOT EXISTS "Public read access for order-files" ON storage.objects
  FOR SELECT USING (bucket_id = 'order-files');

-- Allow authenticated users to upload to order-files bucket
CREATE POLICY IF NOT EXISTS "Authenticated users can upload to order-files" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'order-files' AND auth.role() = 'authenticated');

-- Allow public read access to settings-files bucket
CREATE POLICY IF NOT EXISTS "Public read access for settings-files" ON storage.objects
  FOR SELECT USING (bucket_id = 'settings-files');

-- Allow authenticated users to upload to settings-files bucket
CREATE POLICY IF NOT EXISTS "Authenticated users can upload to settings-files" ON storage.objects
...
CREATE POLICY IF NOT EXISTS "Authenticated users can upload to product-images" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'product-images' AND auth.role() = 'authenticated');

-- Allow authenticated users to delete from product-images bucket
CREATE POLICY IF NOT EXISTS "Authenticated users can delete from product-images" ON storage.objects
