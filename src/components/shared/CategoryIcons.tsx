...
const categoryIcons: Record<string, React.ReactNode> = {
  ...
  'Streaming': <Film className="h-5 w-5" />,
    ...
    <div className="overflow-x-auto pb-2 -mx-4 px-4 scrollbar-thin">
      <div className="flex gap-3 min-w-max">
        {categories.map(category => {
          const icon = categoryIcons[category] || <Layers className="h-5 w-5" />;
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl transition-all duration-300 min-w-[80px] ${
                isSelected
